...
export default {
  ...
  methods : {
    applyCoupon() {
      // Handle the coupon selection event
    ...
    calculateTotal() {
  const discount = (this.cart.subTotal * this.coupon) / 100;

  // Create a new object for this.cart to avoid direct mutation
  this.$emit("cart-updated", {
    cart: {
      ...this.cart,
      discount: discount,
      totalPrice: this.cart.subTotal - discount,
    },
    coupon: this.coupon,
  });
}
